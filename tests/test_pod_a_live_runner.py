...
def _trigger_order(
    *,
    coin: str = "ETH",
    side: str = "A",
    trigger_px: float = 3090.0,
    sz: float = 20.0,
    oid: int = 1,
    time: str = "2026-04-05T08:59:30Z",
    status: str = "open",
) -> dict[str, object]:
    return {
        "time": time,
...
class PodALiveRunnerTests(unittest.TestCase):
    ...
    def test_live_runner_applies_trigger_liquidity_veto_to_pod_a_decisions(self) -> None:
        config = load_config("config/trident.toml")
        config = replace(
            config,
            pod_a=replace(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            trigger_source.write_text(
                json.dumps(_trigger_order()) + "\n",
                encoding="utf-8",
            )
            config.trigger_liquidity.enabled = True
            config.trigger_liquidity.shadow_only = False
            config.trigger_liquidity.source_path = str(trigger_source)
            config.trigger_liquidity.min_cluster_notional_usd = 1_000.0
            config.trigger_liquidity.veto_enabled = True
            config.trigger_liquidity.sizing_enabled = True
            config.trigger_liquidity.confidence_boost_enabled = True
            runner = PodALiveRunner(
                config,
                coins=["ETH"],
                status_path=tmp / "pod_a_live_status.json",
            )
            runner.collector = _FakeCollector(records)  # type: ignore[assignment]
            runner._iter_live_records = fake_iter_live_records  # type: ignore[method-assign]
            journal_path = tmp / "live_journal.jsonl"
            ...
            journal_record = json.loads(journal_path.read_text(encoding="utf-8").strip())
            details = journal_record["signal"]["setup_details"]
            self.assertEqual(details["trigger_liquidity_action"], "veto_entry")
            self.assertEqual(
                details["trigger_liquidity_reason"],
                "adverse_trigger_cascade_risk",
            )
            self.assertEqual(journal_record["signal"]["risk"]["reason"], "trigger_liquidity_veto")
            runtime_status = json.loads((tmp / "pod_a_live_status.json").read_text(encoding="utf-8"))
            self.assertEqual(
                runtime_status["trigger_liquidity"]["signals_by_action"]["veto_entry"],
                1,
            )
            self.assertEqual(
                runtime_status["trigger_liquidity"]["inline_enrichment"]["symbols_enriched"],
                1,
            )
