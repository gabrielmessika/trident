from __future__ import annotations

import json
import tempfile
import unittest
from pathlib import Path

from app.trident_ai import (
    LLM_REPLAY_DECISION_EVENT,
    PAPER_REPLAY_TRADE_CLOSED_EVENT,
    run_trident_ai_failure_pattern_audit,
)
from app.trident_ai.candidate_scan import CANDIDATE_HINT_FIELD


class TridentAIFailurePatternAuditTests(unittest.TestCase):
    def test_failure_pattern_audit_finds_cross_fold_cross_symbol_veto_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            directory = Path(tmpdir)
            fold_a_decisions = directory / "fold_a_decisions.jsonl"
            fold_a_paper = directory / "fold_a_paper.jsonl"
            fold_a_market = directory / "fold_a_market.jsonl"
            fold_b_decisions = directory / "fold_b_decisions.jsonl"
            fold_b_paper = directory / "fold_b_paper.jsonl"
            fold_b_market = directory / "fold_b_market.jsonl"
            report_json_path = directory / "failure.json"
            report_md_path = directory / "failure.md"

            _write_jsonl(
                fold_a_decisions,
                [_decision_record(symbol="BTC", decision_id="btc_loss", timestamp="2026-06-07T12:00:00Z")],
            )
            _write_jsonl(
                fold_b_decisions,
                [_decision_record(symbol="ETH", decision_id="eth_loss", timestamp="2026-06-08T12:00:00Z")],
            )
            _write_jsonl(
                fold_a_paper,
                [_closed_trade("btc_loss", "BTC", opened_at="2026-06-07T12:00:00Z", pnl=-0.25)],
            )
            _write_jsonl(
                fold_b_paper,
                [_closed_trade("eth_loss", "ETH", opened_at="2026-06-08T12:00:00Z", pnl=-0.25)],
            )
            _write_jsonl(
                fold_a_market,
                [
                    _market_record("2026-06-07T12:15:00Z", "BTC", 99.2),
                    _market_record("2026-06-07T13:00:00Z", "BTC", 99.0),
                ],
            )
            _write_jsonl(
                fold_b_market,
                [
                    _market_record("2026-06-08T12:15:00Z", "ETH", 99.2),
                    _market_record("2026-06-08T13:00:00Z", "ETH", 99.0),
                ],
            )

            result = run_trident_ai_failure_pattern_audit(
                decision_journal_paths=(fold_a_decisions, fold_b_decisions),
                paper_journal_paths=(fold_a_paper, fold_b_paper),
                market_input_paths=(fold_a_market, fold_b_market),
                fold_labels=("fold_a", "fold_b"),
                report_json_path=report_json_path,
                report_md_path=report_md_path,
                symbols=("BTC", "ETH"),
                windows_minutes=(15, 60),
                min_trades=2,
                min_loss_trades=2,
                min_loss_folds=2,
                min_loss_symbols=2,
            )

            self.assertEqual(result.summary["trades"], 2)
            self.assertEqual(result.summary["losses"], 2)
            self.assertGreaterEqual(len(result.veto_candidate_buckets), 1)
            pattern_rows = [
                row
                for row in result.veto_candidate_buckets
                if row["bucket_family"] == "pattern"
            ]
            self.assertEqual(len(pattern_rows), 1)
            self.assertEqual(pattern_rows[0]["classification"], "veto_candidate")
            self.assertEqual(pattern_rows[0]["loss_fold_count"], 2)
            self.assertEqual(pattern_rows[0]["loss_symbol_count"], 2)
            self.assertIn("early_adverse_loss", result.summary["failure_labels"])

            report = json.loads(report_json_path.read_text(encoding="utf-8"))
            self.assertEqual(report["kind"], "trident_ai_failure_pattern_audit")
            self.assertIn("TRIDENT-AI Failure Pattern Audit", report_md_path.read_text(encoding="utf-8"))


def _decision_record(*, symbol: str, decision_id: str, timestamp: str) -> dict[str, object]:
    features = {
        "market_cluster": "crypto",
        "spread_bps": 1.0,
        "microprice_dislocation_bps": 1.0,
        "trade_flow_bias": 0.5,
        "book_imbalance": 0.5,
        "vwap_distance_bps": 5.0,
        "realized_vol_short_bps": 10.0,
    }
    hint = {
        "schema_version": "trident_ai_candidate_hint_v6",
        "context_id": f"market_{symbol}_{timestamp}",
        "timestamp": timestamp,
        "symbol": symbol,
        "side": "long",
        "score": 2.0,
        "raw_score": 2.2,
        "directional_score": 1.8,
        "liquidity_score": 1.1,
        "activity_score": 1.0,
        "cost_score": 1.0,
        "edge_quality_score": 1.0,
        "estimated_edge_bps": 24.0,
        "round_trip_cost_bps": 8.0,
        "estimated_net_edge_bps": 16.0,
        "edge_to_cost_ratio": 3.0,
        "reasons": ["ema_bullish", "microprice_aligned", "trade_flow_bias"],
        "pattern_reasons": ["fixture_pattern"],
    }
    return {
        "event_type": LLM_REPLAY_DECISION_EVENT,
        "timestamp": timestamp,
        "symbol": symbol,
        "context": {
            "schema_version": "trident_ai_market_context_v1",
            "context_id": hint["context_id"],
            "as_of": timestamp,
            "symbol": symbol,
            "price": 100.0,
            "regime": "TrendExpansion",
            "features": features,
            CANDIDATE_HINT_FIELD: hint,
            "source": "fixture",
        },
        "proposal": {
            "schema_version": "trident_ai_proposal_v1",
            "decision_id": decision_id,
            "as_of": timestamp,
            "valid_until": "2026-06-07T12:05:00Z",
            "action": "open",
            "symbol": symbol,
            "side": "long",
            "confidence": 0.62,
            "time_horizon_minutes": 60,
            "max_notional_usd": 25.0,
            "max_leverage": 1.0,
            "entry_style": "ioc",
            "invalidation_price": 95.0,
            "stop_bps": 240.0,
            "take_profit_bps": 480.0,
            "time_stop_minutes": 60,
            "rationale_tags": ["fixture"],
            "evidence_ids": [str(hint["context_id"])],
            "risk_notes": ["fixture"],
        },
        "validation": {"accepted": True, "reason": "accepted"},
    }


def _closed_trade(
    decision_id: str,
    symbol: str,
    *,
    opened_at: str,
    pnl: float,
) -> dict[str, object]:
    return {
        "event_type": PAPER_REPLAY_TRADE_CLOSED_EVENT,
        "timestamp": opened_at.replace("12:00:00", "13:00:00"),
        "symbol": symbol,
        "close_reason": "time_stop",
        "trade": {
            "symbol": symbol,
            "side": "long",
            "decision_id": decision_id,
            "opened_at": opened_at,
            "closed_at": opened_at.replace("12:00:00", "13:00:00"),
            "entry_price": 100.0,
            "exit_price": 99.0,
            "notional_usd": 25.0,
            "gross_pnl_usd": pnl + 0.0175,
            "fees_usd": 0.0175,
            "pnl_usd": pnl,
            "close_reason": "time_stop",
            "confidence": 0.62,
        },
    }


def _market_record(timestamp: str, symbol: str, price: float) -> dict[str, object]:
    return {
        "timestamp": timestamp,
        "symbols": [{"symbol": symbol, "price": price}],
    }


def _write_jsonl(path: Path, records: list[dict[str, object]]) -> None:
    path.write_text(
        "\n".join(json.dumps(record, sort_keys=True) for record in records) + "\n",
        encoding="utf-8",
    )


if __name__ == "__main__":
    unittest.main()
