...
class PodABacktestRunnerTests(unittest.TestCase):
    ...
    def test_runner_replays_jsonl_and_writes_output(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("take_profit_bps", result.closed_trade_log[0])
            self.assertIn("break_even_trigger_bps", result.closed_trade_log[0])
            self.assertIn("trailing_activation_bps", result.closed_trade_log[0])
            self.assertIn("trailing_distance_bps", result.closed_trade_log[0])
