...
**Mise à jour opérateur 2026-06-18** : review A/C fraîche `server-data/reviews/20260618T073902Z/` en `PASS` (`mode=live`, cap ordre `200`, reconciliation ready, `0` position/ordre ouvert, `0` conflit ownership). Décisions de clôture P1 : P1-03 est clos comme restauration data + shadow validés, sans guardrail actif ; P1-08 est clos côté P1 en `research_only_no_live_change` ; P1-09 `oil_short_4h_time_gate` est promu en candidat short Pod C actif à risque opérateur accepté. HIP-4 reste `collect_more_data`, sans exécution mainnet ; en revanche le guard `reference_divergence` est promu en mainnet paper uniquement le `2026-06-18` pour tester le veto en flux actif paper.
...
- [x] **P1-03 — Pod C : rétablir la référence externe live**
  ...
  **Contrôle final 2026-06-18** : review `server-data/reviews/20260618T073902Z/p103_external_reference_audit.md` en `PASS`. Runtime Pod C `symbols_enriched=36312`, `symbols_missing_reference=0`, `fetch_error_count=0`; tail journal `1000/1000` setup records enrichis ; shadow `1000/1000`, `live_action_unchanged_false=0`.
  **Clôture P1-03 le 2026-06-18** : la restauration data live et l'instrumentation shadow sont validées. Le guardrail stale/dislocation n'est pas promu et est abandonné comme action P1 faute de validation out-of-sample sur baseline favorable ; il peut rester une piste P2 de diagnostic si de nouveaux trades donnent un signal net. Aucune règle active ne lit `external_reference_shadow_*`.
  **Terminé** : data live Pod C restaurée, export/fetch OK, shadow observé, pas de changement d'entrée/sortie.
...
- [x] **P1-08 — Guard dynamique par symbole / falling knife**
  ...
  **Contrôle final P1-08 du 2026-06-18** : review `server-data/reviews/20260618T073902Z/p108_dynamic_symbol_guard_audit.md` en `PASS`, `2000/2000` records audités avec shadow, `live_action_unchanged_false=0`, états `quarantine=993`, `throttle=776`, `normal=231`. Contrôle full-log local post-déploiement (`>=2026-06-15T16:00Z`) : `10392` records guard, tous en `signal_review`, `0` vrai `signal`, `0` `trade_close`; états `throttle=4184`, `normal=3398`, `quarantine=2810`.
  ...
  **Clôture P1-08 le 2026-06-18** : ne pas promouvoir `throttle`, `quarantine` ni `structural_block_candidate` en production. L'instrumentation fonctionne, mais elle n'a pas encore touché de vrais signaux/trades acceptés et le replay n'apportait qu'un gain faible (`+2.26 USD`) pour une règle très intrusive. Le chantier P1 est donc annulé comme action live ; conserver les champs shadow comme diagnostic P2 seulement.
  **Terminé** : statut final `research_only_no_live_change`.

- [x] **P1-09 — Recherche factorielle from scratch multi-coins/timeframes**
  ...
  **Mise à jour HIP-4 2026-06-18** : review `server-data/hip4/reviews/20260618T095429Z/hip4_outcome_run_review.md` = `collect_more_data` ; total PF `1.0248/1.15`, paper `49` trades / `47` settlements, PnL `+6.2836`, Brier `0.2300`, observer `0` trade. L'audit policy `server-data/hip4/replay_reports/hip4_policy_market_audit_20260618T095116Z.md` est positif post-cutoff (`active_paper` `25` settlements `+40.7368`, PF `1.3973`; `prob_stop_full` `+43.2741`, PF `1.4328`) mais reste trop court et sans confirmation observer. Challenge opérateur accepté côté paper : `block_reference_divergence=true` est promu uniquement en mainnet paper avec seuil `50 bps` et `1` source rejetée minimum, car un cas paper approuvé avec `5` sources rejetées / `83.811 bps` a fini à `-11.555 USDC`. Verdict inchangé : pas de promotion mainnet HIP-4.
  ...
  **Décision initiale P1-09 après replay complet** : ne pas brancher de live trading sans shadow dédié. Critère minimal alors retenu avant promotion dry-run/live : au moins `30-50` signaux shadow indépendants out-of-sample, PF net > `1.15`, DD contenu, contribution positive sur `XYZ:CL` et `XYZ:BRENTOIL`, pas de conflit avec les longs Pod C existants, et replay full-bot mis à jour après collecte shadow. Cette règle de prudence est explicitement dépassée par la décision opérateur du `2026-06-18` ci-dessous pour P1-09 oil seulement.
  ...
  **Fetch / contrôle final P1-09 du 2026-06-18** : review `server-data/reviews/20260618T073902Z/p109_oil_shadow_audit.md` en `PASS`, `4000` records audités, `1241` avec shadow, `would_open=70`, `live_action_unchanged_false=0`, contribution équilibrée (`XYZ:CL=621`, `XYZ:BRENTOIL=620`), régimes observés `mixed/chop/high_vol/uptrend/downtrend`. Contrôle full-log local depuis le déploiement shadow (`>=2026-06-15T19:05:43Z`) : `4231` records oil, `654` `would_open`, tous en `signal_review`; proxy maturé 240m dédupliqué par symbole = `4` trades indépendants, PnL `+2.8927 USD`, PF `1.6005`, `2` wins / `2` losses, `XYZ:CL +1.6944`, `XYZ:BRENTOIL +1.1983`. Le sample reste inférieur au seuil initial, mais il est positif et symétrique.
  **Décision opérateur 2026-06-18 — promotion à risque accepté** : l'opérateur accepte explicitement le risque de promouvoir `oil_short_4h_time_gate` malgré le faible nombre de signaux indépendants. Décision : transformer P1-09 oil de shadow observation-only en candidat short Pod C actif, tout en gardant les rails existants (`live_max_order_notional_usd=200`, risk gate Pod C, allocation cluster oil, `size_multiplier=0.70`, blocklist symboles, reconciliation/preflight).
  **Implémentation promotion P1-09 oil 2026-06-18** : `config/trident.toml` active `pod_c.p109_oil_short_enabled=true` avec `p109_oil_short_min_confidence=0.67`. `app/live/pod_c_live_runner.py` crée un `SignalPreview` + `TradePlan` short `p109_oil_short_4h_time_gate` quand `would_open_p109_oil_short_shadow=true`, uniquement si aucun preview/plan ni position ouverte n'occupe déjà le symbole. Le setup garde un profil d'exit type continuation (`app/trident/pod_c/exits.py`) et un time-stop oil `4h`. Les champs `p109_oil_promoted_*` sont exportés dans le pack d'audit et comptés par `scripts/fetch_trident_data.sh`. Aucun déploiement serveur n'a été lancé par cette modification locale.
  ...
  **Tests promotion 2026-06-18** : `uv run pytest tests/test_pod_c.py::PodCTests::test_pod_c_live_runner_promotes_p109_oil_shadow_to_short_plan tests/test_pod_c.py::PodCTests::test_pod_c_live_runner_adds_p109_oil_shadow_details tests/test_pod_c_oil_shadow.py tests/test_settings.py -q` (`9 passed`) ; `uv run python -m py_compile app/live/pod_c_live_runner.py app/trident/pod_c/oil_shadow.py app/trident/pod_c/exits.py app/settings.py scripts/export_trident_audit_pack.py tests/test_pod_c.py tests/test_pod_c_oil_shadow.py tests/test_settings.py` ; `bash -n scripts/fetch_trident_data.sh` ; `git diff --check`.
  **Terminé** : P1-09 oil `oil_short_4h_time_gate` promu en candidat short actif à risque accepté ; les familles crypto/calendar/gold/silver restent `research_only` ou rejetées ; HIP-4 reste non promu.
...
- [ ] **P1-11 — Découpage micro-régime A/C par coin et contexte**
  **Références** : P1-06, P1-09b/P1-09c, P1-10, audit `tridentAI` du 2026-06-19, levier PnL 2/3.
  **Objectif** : reprendre côté TRIDENT A/C le découpage micro-régime validé en recherche `tridentAI`, afin d'éviter les décisions trop grossières du type "coin bon/mauvais" ou "régime global bon/mauvais". Le cas d'école observé est `HYPE` : mauvais dans certains contextes `range_mid|vol_high`, mais positif dans des sous-régimes plus contrôlés. L'objectif n'est pas de promouvoir une règle live immédiate, mais d'ajouter une étape de replay/audit pour identifier les paires `coin x micro-régime` réellement favorables ou défavorables.
  **Découpage candidat** : calculer au timestamp d'entrée les buckets `range_bucket`, `short_vol_bucket`, `volume_ratio_bucket`, `vwap_bucket`, `microprice_bucket`, puis les composites `range_vol_regime`, `flow_regime`, `micro_regime`, `symbol_range_vol` et `symbol_micro_regime`. Exemples : `range_mid|vol_high`, `flow_elevated|vwap_far`, `HYPE|range_mid|vol_controlled`.
  **Méthode attendue** : reprendre les replays A/C récents et baseline avec ce découpage, puis agréger PnL, PF, WR, stops, drawdown, folds négatifs et concentration symbole par bucket. Tester ensuite uniquement en counterfactual : veto, réduction de taille ou mode défensif par micro-régime. Une promotion live exigerait au minimum une validation baseline + live récente + walk-forward, sans no-trade fold artificiel et sans concentration sur un seul symbole.
  **Garde-fous** : ne pas créer de blacklist globale par coin ; ne pas activer de règle live tant que le support est faible ; pénaliser les buckets avec peu de trades, un seul symbole dominant ou un seul régime de marché ; conserver les règles `research_only` si elles améliorent juin mais détruisent avril/mai.
  **Statut** : ajouté comme étape P1 supplémentaire le 2026-06-19 pour reprise ultérieure dans TRIDENT A/C. Aucun changement live, aucun changement deploy/fetch, aucune modification des pods A/C.
  **Implémentation recherche 2026-06-19** : ajout de `scripts/run_p111_micro_regime_replay.py`, qui enrichit les replays A/C et HIP4 paper avec les snapshots d'entrée, calcule les buckets `range/vol/flow/vwap/microprice` et écrit `p111_micro_regime_replay.{md,json}`, `enriched_trades.csv` et `bucket_rows.csv`. Le runner reste strictement `research_only_no_live_change`.
  **Résultat local 2026-06-19** : rapport `server-data/replay_reports/p111_micro_regime_20260619T164908Z/p111_micro_regime_replay.md`, `498` trades enrichis avec couverture snapshots `100 %` (`451` A/C, `47` HIP4). A/C confirme que l'idée est exploitable mais pas promouvable telle quelle : `range_mid|vol_high` perd `-15.69` sur `7` trades et son veto améliore le PnL agrégé de `+15.69`; `micro_adverse` perd `-26.40` sur `112` trades et une demi-taille counterfactual améliore de `+13.20`. Les meilleurs buckets support sont `range_mid|vol_controlled` (`+50.83`, `19` trades), `flow_normal|vwap_far` (`+61.93`, `25` trades) et `TrendExpansion` (`+364.98`, `150` trades). Les variantes data-minées par bucket améliorent davantage mais restent explicitement à rejouer hors échantillon.
  **Lecture HIP4** : HIP4 est seulement exploratoire dans ce découpage, car les positions dépendent aussi de l'expiry/strike/probabilité du marché outcome. Signal à surveiller : `BUY_NO`/short perd `-78.55` sur `26` trades et `micro_aligned` gagne `+11.05` sur `18` trades, mais aucune règle HIP4 ne doit être changée sans replay dédié outcome-aware.
  **Full replay pré-entry 2026-06-19** : ajout de `scripts/run_p111_micro_regime_full_replay.py`, qui applique les profils micro-régime avant risk gate/exécution dans `FullBotBacktestRunner` avec les caps live. Rapport `server-data/replay_reports/p111_micro_regime_full_replay_20260619T172831Z/p111_micro_regime_full_replay.md` sur `full_bot_live_window_20260524T1605_20260611_no_external_reference.jsonl` (`25742` records, `2026-05-24 -> 2026-06-11`). Baseline A/C cap live `-22.31` (`64` trades, PF `0.65`, WR `34.38 %`). `veto_range_mid_vol_high` rejette `47` plans mais ne change ni trades ni PnL (`-22.31`, delta `+0.00`) ; `half_size_micro_adverse` empire le PnL (`-26.22`, delta `-3.91`, `72` trades) ; la combinaison empire davantage (`-27.59`, delta `-5.28`).
  **Décision P1-11** : ne pas promouvoir. Le counterfactual post-trade montrait une piste, mais le replay complet pré-entry ne l'améliore pas sur la fenêtre récente en pertes. Garder `research_only_no_live_change`; aucune modification `config/trident.toml`, aucun changement deploy/fetch, aucun shadow/live. Prochaine recherche utile : chercher une règle de régime plus structurelle ou un filtre de drawdown/stop path, pas promouvoir ces buckets simples.
  **Tests réalisés localement** : `uv run pytest tests/test_p111_micro_regime_replay.py tests/test_p111_micro_regime_full_replay.py -q` (`4 passed`) ; `uv run python -m py_compile scripts/run_p111_micro_regime_replay.py scripts/run_p111_micro_regime_full_replay.py tests/test_p111_micro_regime_replay.py tests/test_p111_micro_regime_full_replay.py`.
