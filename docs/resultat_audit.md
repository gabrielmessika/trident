...
- [ ] **P1-06 — Régime haussier/baissier Pod A : gate long/short avant entrée**
  ...
  **Prochaine action P1-06** : continuer la collecte shadow ; relancer `scripts/run_p106_regime_shadow_audit.py` après au moins `30` trades fermés avec shadow ou après une vraie fenêtre défensive/bearish. Si le proxy defensive short redevient positif, refaire un replay full-bot gated avant toute promotion.
...
- [ ] **P1-07 — Patterns chartistes multi-timeframe : H&S, EMA cross, order block, cup/handle**
  ...
  **Prochaine action P1-07** : continuer la collecte shadow ; relancer `scripts/run_p107_order_block_shadow_audit.py` après au moins `30` trades fermés avec shadow ou après une vraie fenêtre défensive/bearish. Si le veto-long ou les defensive-shorts redeviennent positifs, refaire un replay full-bot gated A/C avant toute promotion.
  **Terminé quand** : soit le croisement régime + pattern 1h/4h est abandonné, soit il bat la config actuelle sur les deux régimes sans réduire l'activité à un échantillon trop faible et passe une collecte shadow out-of-sample.
...
- [ ] **P1-08 — Guard dynamique par symbole / falling knife**
  ...
  **Prochaine action P1-08** : attendre suffisamment de cycles Pod A, relancer le fetch et vérifier `symbol_guard_shadow_mode=observation_only`, `with_shadow>0`, `symbol_guard_live_action_unchanged_false=0`, puis mesurer pertes évitées vs winners manqués. Critère minimal avant promotion : le shadow live doit montrer une amélioration nette, sans sur-déclencher des throttles sur presque tout le régime récent.
...
- [ ] **P1-09 — Recherche factorielle from scratch multi-coins/timeframes**
  ...
  **Tests réalisés localement** : `uv run pytest tests/test_p109_factor_research_replay.py -q` (`4 passed`) ; `uv run python -m py_compile scripts/run_p109_factor_research_replay.py tests/test_p109_factor_research_replay.py`. Pas de modification fetch/deploy P1-09, car aucun champ `p109_*` n'est journalisé en live/shadow à ce stade.
...
- [ ] **P1-10 — Validation long-historique Hyperliquid API + S3 des patterns calendrier/symbole**
  ...
  **Terminé quand** : soit S3 confirme aussi le rejet, soit une relance S3 complète isole un effet non-calendar appuyé par L2/asset_ctxs et validé hors fenêtre locale.
