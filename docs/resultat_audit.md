...
- [ ] **P1-06 — Régime haussier/baissier Pod A : gate long/short avant entrée**
  ...
  **Prochaine action P1-06** : après plusieurs jours/trades, faire un fetch et mesurer : occurrences `would_block_long`, candidats `would_open_defensive_short_shadow`, PnL réel des longs que le shadow aurait bloqués, et performance contrefactuelle des shorts défensifs sans explosion du trade-count.
  **Étape 8 — Critères de promotion** : aucune activation short live sans confirmation explicite. Candidat promouvable seulement si le shadow montre PF net > 1 sur `defensive_short`, drawdown stable, pas de concentration sur un seul événement, activité acceptable, et si le full-bot gated avec Pod C/routing confirme l'amélioration.
...
- [ ] **P1-07 — Patterns chartistes multi-timeframe : H&S, EMA cross, order block, cup/handle**
  ...
  **Décision actuelle** : ne pas intégrer ces patterns comme règle live directe. Les patterns 15m sont rejetés comme bruit. Le seul candidat à observer est le combo régime P1-06 + order block P1-07b : veto long en régime défensif/baissier avec bearish order block `1h/4h`, et short défensif order block seulement en shadow. Toute activation short basée sur `inverse_cup_handle_breakdown` ou `order_block_bear_retest` reste interdite sans shadow live explicite.
  **Prochaine action** : déployer le shadow P1-07b si on veut collecter en live, puis après plusieurs jours/trades mesurer les occurrences `would_block_long_order_block_shadow`, le PnL réel des longs que le shadow aurait bloqués, les candidats `would_open_defensive_short_order_block_shadow`, et la concentration par date/symbole.
