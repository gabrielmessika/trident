...
class PodBLiveRunner:
    """Runs Pod B on the native Hyperliquid collector using the breakout directional rules."""
        ...
        *,
        ...
        selected_coins = (
            ...
            or symbols_in_allowed_clusters(
                ...
                config.pod_b.allowed_market_clusters,
            )
        )
    ...
    def _process_record(
        ...
        *,
        ...
        previews = self.supervisor.preview_pod_b_signals(snapshots)
        trade_plans = self.supervisor.build_pod_b_trade_plans(snapshots)
    ...
    def _emit_review_summary(
        ...
        *,
        ...
        owned_symbols = self.supervisor.registry.symbols_for(PodName.POD_B)
        accepted_count = sum(1 for decision in risk_decisions if decision.accepted)
        logger.info(
            "Pod B review summary; ts=%s regime=%s tradable_count=%s owned_symbols=%s previews=%s trade_plans=%s accepted=%s opened=%s skipped=%s closed=%s open_positions=%s realized_pnl_usd=%.2f",
            timestamp,
            regime,
            tradable_count,
            owned_symbols,
    ...
    def _refresh_open_positions_without_stream(
        ...
        *,
        ...
        logger.info(
            "Pod B maintenance refresh; idle_seconds=%.1f refreshed_symbols=%s closed=%s",
            idle_seconds,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Pod B directly on the Hyperliquid live collector")
