...
mkdir -p "${RAW_DIR}" "${SNAPSHOT_DIR}" "${TRIGGER_SNAPSHOT_DIR}" "${TRIGGER_SOURCE_DIR}" "${FUNDING_DIR}" "${LOG_DIR}" "${API_DIR}" "${RUNTIME_DIR}" "${DOCKER_DIR}" "${HYDRA_DOCS_DIR}" "${CONFIG_DIR}" "${HIP4_LOG_DIR}" "${HIP4_TESTNET_LOG_DIR}" "${HIP4_MAINNET_PAPER_LOG_DIR}" "${HIP4_MAINNET_LOG_DIR}" "${REPLAY_INPUT_DIR}" "${OUTPUT_DIR}"
